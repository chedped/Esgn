...
- (void)setAssets:(NSArray *)assets
{
    ...
    for (int i = 0; i < [_rowAssets count]; ++i) {
        ...
        if (i < [_overlayViewArray count]) {
            ...
            if (overlayImage == nil) {
                overlayImage = [UIImage imageNamed:@"select_photo_active"];
            }
...
{
    CGFloat width = [self assetWidth];
    int c = 4;//(int32_t)self.rowAssets.count;
    CGFloat totalWidth = c * width + (c - 1) * 4;
    CGFloat startX;
...
-(CGFloat)assetWidth
{
    int c = (int32_t)4;//self.rowAssets.count;
    CGFloat totalWidth = self.bounds.size.width;
    return (totalWidth - ((c-1)*4))/c;
}

+(CGFloat)assetHeightInFrame:(CGRect)bound
{
    int c = 4;
    CGFloat totalWidth = bound.size.width;
    return (totalWidth - ((c-1)*4))/c;
}
@end
