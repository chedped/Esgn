...
- (id)initWithImage:(UIImage *)image
{
    ...
    if (self) {
        ...
        UIImageView *fade = [[UIImageView alloc] initWithFrame:self.bounds];
        fade.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
        fade.backgroundColor = [UIColor colorWithWhite:1.0 alpha:0.4];
        
        UIImageView *img = [[UIImageView alloc] initWithImage:image];
        img.frame = (CGRect){self.bounds.size.width-img.frame.size.width-5,5,img.frame.size.width,img.frame.size.height};
        img.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleBottomMargin;
